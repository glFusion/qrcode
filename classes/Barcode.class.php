<?php
/**
 * Class to handle creating barcodes.
 *
 * @author      Lee Garner <lee@leegarner.com>
 * @author      David S. Tufts (davidscotttufts.com)
 * @copyright   Copyright (c) 2010-2020 Lee Garner <lee@leegarner.com>
 * @copyright   Copyright (c) 2003 David S. Tufts
 * @package     qrcode
 * @version     v1.1.0
 * @since       v1.1.0
...
class Barcode extends Code
{
    ...
    /** Indicate that the code will be printed, causes padding.
     * @var boolean */
    private $print = true;
     ...
     *
     * @param   array   $params     Parameters. At least `data` must be provided
     */
    public function __construct($params)
    {
        ...
        if (isset($params['code_type'])) {
            $this->setCodeType($params['code_type']);
        }
        if (isset($params['orientation'])) {
            $this->setOrientation($params['orientation']);
        }
        $this->filename = md5(@serialize($params)) . '.png';
     ...
     *
     * @param   string  $value      Barcode type
     * @return  object  $this
     */
    private function setCodeType($value)
    {
        switch ($value) {
        case 'code128':
        case 'code128b':
    ...
    /**
     * Set the orientation for the image, vertical or horizontal (default).
     *
     * @param   string  $value      Either `horizontal` or `vertical`
     * @return  object  $this
     */
    private function setOrientation($value)
    {
        if ($value == 'vertical') {
...
// This function call can be copied into your project and can be made from anywhere in your code
     ...
     * Also sets `have_image` if image created successfully
     *
     * @return  boolean     True if image created, False if not
     */
    ...
    // Translate the $text into barcode the correct $code_type
        //if (in_array(strtolower($this->code_type), array("code128", "code128b"))) {
        ...
        case 'code128a':
            ...
            $code_array = array(
                ...
                "<"=>"322112", "="=>"322211", ">"=>"212123", "?"=>"212321",
                "@"=>"232121", "A"=>"111323", "B"=>"131123", "C"=>"131321",
                "D"=>"112313", "E"=>"132113", "F"=>"132311", "G"=>"11313",
                "H"=>"231113", "I"=>"231311", "J"=>"112133", "K"=>"112331",
                "L"=>"132131", "M"=>"113123", "N"=>"113321", "O"=>"133121",
                "P"=>"313121", "Q"=>"211331", "R"=>"231131", "S"=>"213113",
                "T"=>"213311", "U"=>"213131", "V"=>"311123", "W"=>"311321",
                "X"=>"331121", "Y"=>"312113", "Z"=>"312311", "["=>"332111",
                "\\"=>"314111", "]"=>"221411", "^"=>"431111", "_"=>"111224",
                "NUL"=>"111422", "SOH"=>"121124", "STX"=>"121421",
                "ETX"=>"141122", "EOT"=>"141221", "ENQ"=>"112214",
                "ACK"=>"112412", "BEL"=>"122114", "BS"=>"122411",
                "HT"=>"142112", "LF"=>"142211", "VT"=>"241211", "FF"=>
                "221114", "CR"=>"413111", "SO"=>"241112", "SI"=>"134111",
                "DLE"=>"111242", "DC1"=>"121142", "DC2"=>"121241",
                "DC3"=>"114212", "DC4"=>"124112", "NAK"=>"124211",
                "SYN"=>"411212", "ETB"=>"421112", "CAN"=>"421211",
                "EM"=>"212141", "SUB"=>"214121", "ESC"=>"412121",
                "FS"=>"111143", "GS"=>"111341", "RS"=>"131141",
                "US"=>"114113", "FNC 3"=>"114311", "FNC 2"=>"411113",
                "SHIFT"=>"411311", "CODE C"=>"113141", "CODE B"=>"114131",
                "FNC 4"=>"311141", "FNC 1"=>"411131", "Start A"=>"211412",
        ...
        case 'code25':
            ...
            $code_array2 = array(
                "3-1-1-1-3", "1-3-1-1-3", "3-3-1-1-1", "1-1-3-1-3",
                "3-1-3-1-1", "1-3-3-1-1", "1-1-1-3-3", "3-1-1-3-1",
                "1-3-1-3-1", "1-1-3-3-1",
            );
        ...
        case 'codabar':
            ...
            $code_array2 = array(
                "1111221", "1112112", "2211111", "1121121", "2111121",
                "1211112", "1211211", "1221111", "2112111", "1111122",
        ...
        if ($this->filepath=="") {
            ...
            $this->have_image = imagepng($image);
            imagedestroy($image);
        } else {
            $filepath = $this->filepath . '/' . $this->filename;
            $this->have_image = imagepng($image, $filepath);
            imagedestroy($image);
        }
    ...
    /**
     * Get the HTML to display a barcode image.
     *
     * @uses    parent::file_exists()
     * @uses    parent::getURL()
    ...
    public function getHTML($classes=array())
    {
        if ($this->have_image || $this->createImage()) {
            ...
            $html = '<div class="qrcode ' . $cls . '">' . LB .
                COM_createImage($this->getURL(), 'Barcode') .
                '</div>' . LB;
        } else {
            $html = '';
        }
